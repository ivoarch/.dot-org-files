...
alias pac-mirror-update='sudo mv /etc/pacman.d/mirrorlist /etc/pacman.d/mirrorlist.backup+`date +y%m%d`; sudo reflector -l 10 --sort rate --save /etc/pacman.d/mirrorlist'
