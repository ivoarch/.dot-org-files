...
1. =C-t c= run-or-raise Urxvt (Screen)
2. =C-t E= run-or-raise Emacs GUI
3. =C-t e= run-or-raise to the Emacs inside screen session **(Ugly Hack? - Please fix me!)** Set window 1 for emacs
in your =.screenrc= config.
